...
var lensFlareSel;
    ...
    function readDataURI(dataURI) {
        // Extract the MIME type and the data from the Data URI
        const [, mimeType, type, data] = dataURI.match(/^data:(.*?);(.*?),(.*)$/);
        console.log(data);

        // Decode the data from base64 or percent-encoding
...
var Ed3d = {
  ...
  'material'            : {
    ...
    'transparent2' : new THREE.MeshBasicMaterial({
      color: 0x0E7F88,
      transparent: true,
      wireframe: true,
      opacity: 0.05
    ...
    'solution' : new THREE.MeshBasicMaterial({
      color: 0xFA6D00,
      transparent: true,
      opacity: 0.1,
      wireframe: true
  ...
  'loadDatasFromFile' : function() {
      ...
      $.getJSON(readDataURI(this.jsonPath), function(data) {
  ...
  'loadDatas' : function(data) {
      ...
      $.each(list, function(key, val) {
          ...
          system = System.create(val, true);
        if(system != undefined) {
          if(val.cat != undefined && Array.isArray(system)) Ed3d.addObjToCategories(system,val.cat);
