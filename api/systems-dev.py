...
origins = [
    "https://edgis-preprod.hobeika.fr",
...
async def fetch_bodies_test_from_db(name_or_id: str):
    import psycopg
    ...
    if name_or_id.isdigit() or (
        ...
        query = """
            SELECT b.*
            FROM bodies_test b, systems_big s
            WHERE LOWER(s.name) = LOWER(%s)
            AND s.id64 = b.system_id64
            ORDER BY b.body_id;
        """
...
@app.get("/bodies_test", include_in_schema=True)
async def bodies(
    name_or_id: str = Query(..., description="The name or id64 of the system")
):
    result = await fetch_bodies_test_from_db(name_or_id)
    if result is None:
        return JSONResponse(content={"error": "System not found"}, status_code=404)
    return result
...
@app.get("/")
def read_index():
    return FileResponse(os.path.join("static", "index-dev.html"))
